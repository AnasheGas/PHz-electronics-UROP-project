...
# Load your netlist from SPICE, you can also manually edit it in the file or in the Python Script and define it as a string
netlist = """

O1 N001 N008 N004 N006 Stub1
O2 N002 N005 N003 N007 roundTline
C4 N004 N006 0.468µ
O5 N010 N009 0 N011 Stub2
C11 0 N011 0.468µ
readout1 0 N011 {Rread}
X§X1 N003 N007 N001 N008 single_antenna_normalized params: amp = {ampY} del = 105 phase = {phaseTop} sigdur = {sigdurTop} d = {dTop}
X§X2 N005 N002 N010 N009 single_antenna_normalized params: amp = {ampX} del = {105 + delt} phase = {phaseTop} sigdur = {sigdurTop} d = dTop
...
.param ampY = 3.54E-4*dTop
.param ampX = 1.73E-4*dTop
...
"""
# Save file of net/cir file, your file will be in different location, so change the path accordingly
path = r"C:\Users\anash\OneDrive\Desktop\QNN REPO\PHz-electronics-UROP-project\2026_nanoantenna_subckt_files\2_couple_antennas_8-6-2026Test.net"
with open(path, "w") as f:
    f.write(netlist)

# Set Variable with path of LTSPICE executable file on local comupter, your file will be in different location, so change the path accordingly
LTSPICE = r"C:\Users\anash\AppData\Local\Programs\ADI\LTspice\LTspice.exe"

# Use subprocess to run LTSPICE in batch mode with the netlist file, raw file will update to latest run
result = subprocess.run([LTSPICE, "-b", "-Run", path], check=True, capture_output=True, text=True)

# save raw file path to variable
raw = PyLTSpice.RawRead(r"C:\Users\anash\OneDrive\Desktop\QNN REPO\PHz-electronics-UROP-project\2026_nanoantenna_subckt_files\2_couple_antennas_8-6-2026Test.raw")

# Use PyLTSpice methods to extract data and plot, info on methods in ReadMe
print(raw.get_trace_names())

plt.figure(figsize=(10, 6))

plt.plot(raw.get_trace("time"), raw.get_trace("I(readout1)"), label="Readout Current", color='blue')


plt.xlabel("time (s)")
plt.ylabel("Readout Current")
plt.title("Readout Current vs Time")
plt.grid(True)
plt.legend()
plt.show()